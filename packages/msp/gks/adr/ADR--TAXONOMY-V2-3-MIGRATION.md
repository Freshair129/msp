...
- Taxonomy tables in `CLAUDE.md` (×3) and `packages/gks/docs/KNOWLEDGE-TYPES.md`
...
3. **Migration script** — Node.js script under `packages/msp/scripts/msp/migrate-frame-to-framework.mjs` that performs renames + ref updates with a `--dry-run` flag
...
- **The repo's own atom tree under `packages/gks/gks/` does not currently contain a `frame/` subdirectory** beyond `FRAMEWORK--FOUR-LAYERS.md`. The directory move there is a single file; in `packages/msp/gks/` it covers 8 files. Empty `frame/` directories are removed by the script.
